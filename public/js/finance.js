...
function renderTransactions(transactions) {
  ...
  transactionList.innerHTML = transactions.map(t => {
    ...
    return `
      <div class="bg-surface-card p-6 border border-ui transition-all hover:border-[#F8DE22] group shadow-sm" style="border-radius:1.5rem">
        <div class="flex justify-between items-center">
          <div class="flex items-center gap-5">
            <div class="flex items-center justify-center text-3xl" style="margin-right:0.5rem">
          ...
          <div class="flex items-center gap-6">
            ...
            <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
              <button class="p-2.5 hover-color-primary transition-colors" onclick="editTransaction('${escapeHtml(t.id)}')" title="Editar">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
              </button>
              <button class="p-2.5 hover-color-danger transition-colors" style="margin-left:0.75rem" onclick="deleteTransaction('${escapeHtml(t.id)}')" title="Eliminar">
