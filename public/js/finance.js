...
function renderTransactions(transactions) {
  if (!Array.isArray(transactions) || transactions.length === 0) {
    transactionList.innerHTML = `
      <div class="p-10 rounded-[2.5rem] bg-surface-input border-2 border-dashed border-ui text-center">
  ...
  transactionList.innerHTML = transactions.map(t => {
    ...
    return `
      <div class="bg-surface-card p-6 rounded-[2rem] border-2 border-ui transition-all hover:border-[#F8DE22] group shadow-sm">
        <div class="flex justify-between items-center">
          <div class="flex items-center gap-5">
            <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl bg-surface-input border border-ui shadow-inner">
              ${escapeHtml(t.categories?.icon) || '📦'}
            </div>
            <div>
              <h5 class="text-base font-black text-ui uppercase tracking-tighter italic">${escapeHtml(t.description) || 'Sin descripción'}</h5>
          ...
          <div class="flex items-center gap-6">
            ...
            <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
              <button class="p-2.5 bg-surface-input rounded-xl hover-color-primary shadow-sm transition-colors" onclick="editTransaction('${escapeHtml(t.id)}')" title="Editar">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
              </button>
              <button class="p-2.5 bg-surface-input rounded-xl hover-color-danger shadow-sm transition-colors" onclick="deleteTransaction('${escapeHtml(t.id)}')" title="Eliminar">
